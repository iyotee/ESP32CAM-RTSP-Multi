 ...
 *
 * @modifications:
 * 2025-08-28	JCZD	added BSSID-related stuff
 */
// config.h.example
// Configuration file for sensitive information and system parameters
// edit and save as config.h
...
// Default WiFi channel and BSSID (used when WIFI_USE_BSSID is not defined)
#ifndef WIFI_CHANNEL
#define WIFI_CHANNEL 0 // 0 = auto-selection
#endif

#ifndef WIFI_BSSID
const uint8_t WIFI_BSSID[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; // Default BSSID
#endif

// ===== STATIC IP CONFIGURATION =====
// Choose between DHCP (automatic) or static IP configuration
// #define WIFI_USE_STATIC_IP
// DHCP is used by default for automatic IP assignment

// EXAMPLE: To enable static IP, change the line above to:
// #define WIFI_USE_STATIC_IP
// Then modify the IP configuration below to match your network.

// ===== EXPLICIT ACCESS POINT SELECTION =====
// for more complex network setups, it may be desirable to specify which
// AP the camera should connect to ; in such a case, uncomment the define
// directive below and

// BSSID is used by default for explicit AP selection ⚠️ MODIFY THESE VALUES IF USING BSSID
// #define WIFI_USE_BSSID
...
// Static IP for simple networks (automatic AP selection)
...
//
// Static IP configuration for more complex networks and epxlicit AP selection
//
#include <IPAddress.h>

#define LOCAL_CAMERA_ID 77 // last byte of IP address
#define MY_PREFERRED_AP    // connect to this AP (config below)

// example of AP config
#ifdef MY_PREFERRED_AP
#define SUBNET_INT 1 // subnet selection
const uint8_t WIFI_CHANNEL = 4; // set to 0 for auto-selection
const uint8_t WIFI_BSSID[6] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55};
#endif

const IPAddress staticIP(172, 16, SUBNET_INT, LOCAL_CAMERA_ID);
const IPAddress gateway(172, 16, SUBNET_INT, 1);
const IPAddress subnet(255, 255, 255, 0);

#define HAS_PRIMARY_DNS
const IPAddress primaryDNS(8, 8, 8, 8); // Optional
// #define HAS_SECONDARY_DNS	// takes precendence over HAS_PRIMARY_DNS
// const IPAddress secondaryDNS(8, 8, 4, 4); // Optional

//
// end of section relative to AP selection
//
#endif
#endif

// ===== NTP CLOCK CONFIGURATION =====
// NTP server for synchronization (comment out to disable NTP sync)
...
// NTP connection timeout (milliseconds)
...
#define RTSP_SERVER_NAME "ESP32CAM-RTSP-Multi/1.1" // TODO redundancy in version/label is BAD!
...
#define LOG_LEVEL LOG_INFO // DEBUG level pour voir tous les logs
